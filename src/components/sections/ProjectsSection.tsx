...
const Card = ({ project, i, progress, range, targetScale }: CardProps) => {
  ...
  return (
    <div ref={containerRef} className="h-[85vh] flex items-center justify-center sticky top-24 md:top-32" style={{ top: `calc(6rem + ${i * 28}px)` }}>
      <motion.div 
        style={{ scale }}
        className="flex flex-col relative w-full max-w-6xl mx-auto rounded-[40px] sm:rounded-[50px] md:rounded-[60px] border-2 border-[#D7E2EA] bg-[#0C0C0C] p-4 sm:p-6 md:p-8 transform-gpu"
      >
        <div className="flex flex-col md:flex-row justify-between md:items-end mb-6 sm:mb-8 md:mb-10 gap-6">
          <div className="flex flex-col md:flex-row gap-4 md:gap-8 items-start md:items-end">
            <span className="font-black text-[#D7E2EA] text-[clamp(3rem,10vw,100px)] leading-none block">
              {project.num}
            </span>
            <div className="flex flex-col pb-1 md:pb-3">
              <span className="text-[#D7E2EA]/60 uppercase tracking-widest text-xs sm:text-sm mb-1">{project.label}</span>
              <h3 className="text-[#D7E2EA] font-medium uppercase text-[clamp(1.5rem,3vw,2.5rem)] leading-none">{project.name}</h3>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
          <div className="w-full sm:w-[40%] flex flex-col gap-4 sm:gap-6">
            <div className="w-full overflow-hidden rounded-[40px] sm:rounded-[50px] md:rounded-[60px] relative" style={{ height: "clamp(130px, 16vw, 230px)" }}>
              <motion.img 
            ...
            <div className="w-full overflow-hidden rounded-[40px] sm:rounded-[50px] md:rounded-[60px] relative" style={{ height: "clamp(160px, 22vw, 340px)" }}>
              <motion.img 
                style={{ scale: imageScale }}
          ...
          <div className="w-full sm:w-[60%] overflow-hidden rounded-[40px] sm:rounded-[50px] md:rounded-[60px] relative" style={{ height: "clamp(300px, 40vw, 594px)" }}>
            <motion.img 
              style={{ scale: imageScale }}
